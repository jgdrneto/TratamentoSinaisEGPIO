...
private:
   ...
   */
  static vector<string> obterAtributos(vector<string> informacoes){
    ...
    for(string p : processosString){
      ...
      if(!informacoes.empty()){
        ...
        if(atributos.size()!=4){
          ...
          unsigned int memoria = (unsigned int)std::stoi (atributos[3],nullptr,0);
...
public:
   ...
   */
  static Processo& buscaPorId(string id,vector<Processo>& processos){
   ...
   */
  static bool contemUsuario(vector<string> usuarios,string s ){
   ...
   */
  static vector<string> obterUsuariosDistintos(vector<Processo> processos){
  ...
  static int contarProcessosDoUsuario(string usuario, vector<Processo> processos){
